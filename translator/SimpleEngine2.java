...
public class SimpleEngine2 {
	...
	public static void main(String[] args) throws Exception {
		...
		fileNames.remove(1);
		Utility.sendFilesXInFolderYToVM("/var/FileDPs_minimum/", fileNames, srcIP, vmIPs.get(4));
		...
		outputFiles.add("mProjectPPmImgtbl50.o2.79.0.fits");
		outputFiles.add("mProjectPPmImgtbl50.o2.79.0_area.fits");
		...
		outputFiles.clear();
		outputFiles.add("mOverlaps161.o1.79");
		outputFiles.add("mMergeImgs156.o1.79");
		mMergeImgs += commandToSend(outputFiles, vmIPs.get(1));
		mMergeImgs += commandToSend(outputFiles, vmIPs.get(2));
		mMergeImgs += commandToSend(outputFiles, vmIPs.get(3));
		outputFiles.remove(1);
		mMergeImgs += commandToSend(outputFiles, vmIPs.get(4));
		...
		String mBgExec = "mkdir tmp; mv mProjectPPmImgtbl50.o2.79.0.fits tmp; mv mProjectPPmImgtbl50.o2.79.0_area.fits tmp; "
				+ "mBgExec -p tmp mMergeImgs156.o1.79 mBgModel284.o1.79 .; rm -r -f tmp;";
		...
		// ///////////////////////////////////// mBgExec
		mBgExec = "mkdir tmp; mv mProjectPPmImgtbl56.o2.79.0.fits tmp; mv mProjectPPmImgtbl56.o2.79.0_area.fits tmp;"
				+ "mBgExec -p tmp mMergeImgs156.o1.79 mBgModel284.o1.79 .; rm -r -f tmp;";

		outputFiles.clear();
		outputFiles.add("mProjectPPmImgtbl56.o2.79.0.fits");
		outputFiles.add("mProjectPPmImgtbl56.o2.79.0_area.fits");
		mBgExec += commandToSend(outputFiles, vmIPs.get(0));

		executeRemotely(mBgExec, vmIPs.get(1));

		mBgExec = "mkdir tmp; mv mProjectPPmImgtbl62.o2.79.0.fits tmp; mv mProjectPPmImgtbl62.o2.79.0_area.fits tmp;"
				+ "mBgExec -p tmp mMergeImgs156.o1.79 mBgModel284.o1.79 .; rm -r -f tmp;";
		...
		outputFiles.add("mProjectPPmImgtbl62.o2.79.0_area.fits");
		mBgExec += commandToSend(outputFiles, vmIPs.get(0));
		executeRemotely(mBgExec, vmIPs.get(2));

		mBgExec = "mkdir tmp; mv mProjectPPmImgtbl68.o2.79.0.fits tmp; mv mProjectPPmImgtbl68.o2.79.0_area.fits tmp;"
				+ "mBgExec -p tmp mMergeImgs156.o1.79 mBgModel284.o1.79 .; rm -r -f tmp;";
		...
		outputFiles.add("mProjectPPmImgtbl68.o2.79.0_area.fits");
		mBgExec += commandToSend(outputFiles, vmIPs.get(0));
		executeRemotely(mBgExec, vmIPs.get(3));

		String mAddMJPEG = "mkdir tmp; mv mProjectPPmImgtbl62.o2.79.0.fits tmp; mv mProjectPPmImgtbl62.o2.79.0_area.fits tmp; "
				+ "mv mProjectPPmImgtbl50.o2.79.0.fits tmp; mv mProjectPPmImgtbl50.o2.79.0_area.fits tmp; mv mProjectPPmImgtbl68.o2.79.0.fits tmp; "
				+ "mv mProjectPPmImgtbl68.o2.79.0_area.fits tmp; mv mProjectPPmImgtbl56.o2.79.0.fits tmp; mv mProjectPPmImgtbl56.o2.79.0_area.fits tmp; "
				+ "mAdd -p tmp mMergeImgs156.o1.79 template.hdr mAdd356.o1.79; rm -r -f tmp; "
				+ "mJPEG -gray mAdd356.o1.79.fits 0s max gaussian-log -out mJPEG378.o1.79.jpeg;";

		executeRemotely(mAddMJPEG, vmIPs.get(0));

		String downloadJPEG = "sshpass -p '" + Utility.passwdToVMs + "' scp -o StrictHostKeyChecking=no view@" + vmIPs.get(0) + ":"
				+ Utility.pathToFileDPsFolder + "mJPEG378.o1.79.jpeg /home/andrey/tmp/2015_12_07_reverseEng/";
		Utility.executeShellCommand(downloadJPEG);
	}
