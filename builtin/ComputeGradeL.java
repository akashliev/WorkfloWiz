 ...
 * 
 */
public class ComputeGradeL extends Workflow {
	public ComputeGradeL(String instanceid) {
	...
	public void execute(String runID, boolean registerIntermediateDPs) throws Exception {
		...
		if (inputPortID_to_DP.get("i1") instanceof FileDP) {
			...
			File f = new File(pathToModelFile);
			if(!f.exists())
				pathToModelFile = "/home/view/FileDPs/" + modelFileName;
		}
		...
		try {
			String command = "cd ; cd software/trunk/bin; ./mahout runlogistic --input " + pathToDrivingSummary + " --model " + pathToModelFile
					+ " --scores";
			...
			String profile = Utility.readFromAFile(pathToDrivingSummary);
			String outFileName = instanceId + ".o1." + runID;
			String pathToOutFile = Utility.pathToFileDPsFolder + outFileName;
			String result = textAnalyzer.createDrivingSummary(profile, shellOutput);
			System.out.println("||||||||||||||||||||||||||| ComputeGradeL writing to file " + pathToOutFile);
	...
	public boolean readyToExecute() {
		if (inputPortID_to_DP == null) {
			System.out.println("rrrrrrrrrrrrrrrrrreturning false");
			return false;
		}
		return true;
	}
