 ...
 * 
 */
public class ShuffleSleep extends Workflow {
	public ShuffleSleep(String instanceid) {
	...
	public void execute(String runID, boolean registerIntermediateDPs) throws Exception {
		...
		Utility.reportToLoggerNode("++00 " + instanceId);
		...
		Utility.reportToLoggerNode("++11 " + instanceId);
		executionStatus = ExecutionStatus.inProcessOfExecution;

		try {
			ArrayList<String> allWords = new ArrayList<String>();
			Utility.reportToLoggerNode("++22 " + instanceId);
			File file = new File(pathToFile);
			Scanner sc = new Scanner(new FileInputStream(file));
			while (sc.hasNext())
				allWords.add(sc.next());
			Utility.reportToLoggerNode("++33 " + instanceId);
			...
			Utility.reportToLoggerNode("++44 " + instanceId);
			output.close();
			Thread.sleep(20000);
			FileDP resultDP = new FileDP(outputFileName);
			outputPortID_to_DP.put("o1", resultDP);
			Utility.reportToLoggerNode("++55 " + instanceId);
